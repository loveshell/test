...
public class GeneratorHbase extends Generator {
	...
	public static class CodeInputFormat implements InputFormat<IntWritable, IntWritable> {
		...
		public InputSplit[] getSplits(JobConf job, int numSplits) throws IOException {
			return new CustomInputSplit[] { new CustomInputSplit(job.getInt(GeneratorHbase.GENERATL_REDUCENUM, 1)) };
		}

		public RecordReader<IntWritable, IntWritable> getRecordReader(InputSplit split, JobConf job, Reporter reporter)
				throws IOException {
			return new CodeReader(job.getInt(GeneratorHbase.GENERATL_REDUCENUM, 1));
		}
	}
	...
	public static class GenerateMark implements Reducer<IntWritable, IntWritable, Text, CrawlDatum> {
		...
		public void configure(JobConf job) {
			...
			reduceNum = job.getInt(GeneratorHbase.GENERATL_REDUCENUM, 1);
	...
	public Path[] generate(Path dbDir, Path segments, int numLists, long topN, long curTime, boolean filter,
		...
		// NutchFetchStatis.RunGenStatis(getConf());
		// NutchInfo.upGenQueue(NutchFetchStatis.getFetchQueue());
		// NutchInfo.upGenUrl(NutchFetchStatis.getFetchUrl());
